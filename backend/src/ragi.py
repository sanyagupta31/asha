    ...
    def get_context(self, query: str, location: str = "") -> str:
        ...
        if local_jobs:
            context_sections.append(self._format_section(
                ...
                [f"- {job['title']} ({job['location']}): {job['description']}" for job in local_jobs]
            ))
        
        if live_jobs:
            context_sections.append(self._format_section(
                "Live Job Listings",
                [f"- {job['title']} at {job['company']} ({job['location']}): {job['description']}" for job in live_jobs]
            ))
        
        if live_events:
            context_sections.append(self._format_section(
                "Upcoming Events",
                [f"- {event['name']} at {event['venue']} ({event['date']}): {event['description']}\n  {event['url']}" 
        ...
        if sessions:
            context_sections.append(self._format_section(
                "Local Sessions",
                [f"- {session['title']} on {session['date']}: {session['description']}" for session in sessions]
            ))
